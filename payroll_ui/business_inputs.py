...
REVIEW_ACTIONS = {"APPROVE": "APPROVED", "REJECT": "REJECTED", "REQUEST_MORE_INFO": "NEEDS_REVIEW"}
...
class BusinessInputService:
    ...
    def import_results(self, input_type: str, period: str, path: str, submitted_by: str) -> list[dict]:
        ...
        # A newly selected final result table becomes the new candidate
        # authority for this period. Historical imports remain queryable and
        # any bound old result will make its Run/candidate require review.
        for prior in self.store.list_business_inputs():
            if prior.get("input_type") == input_type and prior.get("period") == resolved_period and prior.get("status") in {"NEEDS_REVIEW", "APPROVED", "APPLIED"}:
        ...
        for row in records:
            item = {
                ...
                "source_file_hash": version["sha256"], "source_row": row.row, "submitted_by": submitted_by.strip(), "submitted_at": now(),
                "status": "NEEDS_REVIEW", "payload": row.payload, "evidence": {**row.evidence, **version, "batch_id": batch_id},
                "reviewed_by": "", "reviewed_at": "", "review_note": "", "linked_run_id": "", "created_at": now(), "updated_at": now(),
            }
    ...
    def bind_to_run(self, input_id: str, run: dict) -> tuple[dict, dict]:
        item = self.store.get_business_input(input_id)
    ...
    def current(self, input_id: str, expected_hash: str = "") -> bool:
        ...
        if item.get("source_file_hash") and source.is_file():
            try:
