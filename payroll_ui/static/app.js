...
async function saveCoreRules(returnRunId = "") {
  try {
    ...
    showMessage(`核心规则已保存为新版本${savedId ? `：${savedId}` : ""}。历史核算 不会自动改变。`, "success");
...
async function savePartTimeRates(returnRunId = "") {
  try {
    ...
    const savedId = versionList(saved)[0]?.id;
    showMessage(`兼职单价已保存为新版本${savedId ? `：${savedId}` : ""}。历史核算 不会自动改变。`, "success");
...
// ---------------------------------------------------------------------------
// 历史班型规则：仅用于解释旧核算；新核算 统一由“核心规则面板”绑定规则包。
