...
function issuesPage() {
  ...
  const actionSummary = actions.length ? `<div class="action-summary"><strong>需要完成 ${actions.length} 个核查动作</strong><span class="muted">后台保留 ${rows.length} 个业务问题和 ${fieldCount} 条审计记录，便于逐项查看证据。</span><div class="small">${actions.map((action) => `${escapeHtml(action.title)}：${action.teacher_count} 位教师`).join(" · ")}</div></div>` : "";
